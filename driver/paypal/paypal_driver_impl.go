...
func init() {
	registerBuilder("qrcode", driver.LinkTypeCodeUrl, func(b builder.Builder, c Config) (driver.Driver, error) {
		driver, err := newDriver(b, c)
		return &Driver{_Driver: driver}, err
	})
}
...
func (d Driver) RefundPayment(ctx context.Context, req driver.CreateRefundRequest) (info driver.RefundInfo, err error) {
	...
	return d.refundToRefundInfo(_resp, req.ChannelData), nil
}
...
func (d Driver) QueryRefund(ctx context.Context, req driver.QueryRefundRequest) (info driver.RefundInfo, ok bool, err error) {
	...
	return d.refundToRefundInfo(_resp, req.ChannelData), true, nil
}

func (d Driver) ParseCallbackRequest(ctx context.Context, req driver.CallbackRequest) (cbinfo driver.CallbackInfo, err error) {
	event, raw, err := d.parseAndVerifyWebhook(ctx, req.Request)
	if err != nil {
		return
	}
	...
	case "CHECKOUT.ORDER.APPROVED":
		...
		var ord paypal.OrderResponse
		ord, err = d.captureOrder(ctx, res.Id)
		if err != nil {
			return
		}

		info := d.orderToPaymentInfo(ord, "")
	...
	case "CHECKOUT.PAYMENT-APPROVAL.REVERSED":
		...
		cbinfo.Type = driver.CallbackTypePayment
		cbinfo.PaymentInfo = &driver.PaymentInfo{
	...
	case
		...
		info := d.captureToPaymentInfo(cap.SupplementaryData.RelatedIds.OrderId, cap)
		cbinfo.PaymentInfo = &info
		cbinfo.Type = driver.CallbackTypePayment
	...
	case "PAYMENT.CAPTURE.REFUNDED":
		...
		cbinfo.Type = driver.CallbackTypeRefund
		cbinfo.RefundInfo = &driver.RefundInfo{
...
func (d Driver) orderToPaymentInfo(ord paypal.OrderResponse, fallbackPaymentId string) driver.PaymentInfo {
	if len(ord.PurchaseUnits) > 0 && len(ord.PurchaseUnits[0].Payments.Captures) > 0 {
		return d.captureToPaymentInfo(ord.Id, ord.PurchaseUnits[0].Payments.Captures[0])
	}
...
func (d Driver) refundToRefundInfo(resp paypal.RefundResource, cd string) driver.RefundInfo {
	info := driver.RefundInfo{
		PaymentId: "",
		RefundId:  cmp.Or(resp.CustomId, resp.InvoiceId),
